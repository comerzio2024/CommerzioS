...
- **Category Filter Bars** (2025-11-24): Browse Services (/services) and Saved Listings (/saved, /favorites) pages feature CategoryFilterBar component at the top for quick category filtering. The filter bar displays all categories with service counts, highlights the selected category, and updates the URL with query parameters. On Browse Services, the filter bar works alongside the sidebar filters (single category selection from bar + multi-select from sidebar checkboxes). On Saved Listings, it filters saved services and shows empty state when no services match the selected category.
